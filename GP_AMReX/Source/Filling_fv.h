...
void init_phi_cyl(MultiFab& phi_new)
{
	for (MFIter mfi(phi_new); mfi.isValid(); ++mfi)
	{
		...
		{	
			...
			#if (AMREX_SPACEDIM > 2)
			if(((i-cyl_X_cent)*(i-cyl_X_cent) + (j-cyl_Y_cent)*(j-cyl_Y_cent)) < cyl_rad*cyl_rad && cyl_Z_strt<=k && k<=cyl_Z_end)
			{
				phiNew(i,j,k,cyl_comp) = 1.0;
			}
			#endif 
...
void init_phi_cube(MultiFab& phi_new)
{
	for (MFIter mfi(phi_new); mfi.isValid(); ++mfi)
	{
		...
		{	
			#if (AMREX_SPACEDIM==2)
			if ((cube_X_strt <= i && i < cube_X_end) &&
    			(cube_Y_strt <= j && j <cube_Y_end))
			{
        ...
        while(Phase_filled!=1){
            ...
            for(int x=0; x<ncellx; x++){
                for(int y=0; y<ncelly; y++){
                    if ((n[i]-x)*(n[i]-x) +(m[i]-y)*(m[i]-y) <= Vsz*Vsz) {
                    gidy1 = x*ncelly + y;
                    flag[gidy1] = 1;
                }
            }
...
void init_phi_voronoi_3D(MultiFab& phi_new){
	
		int Vx_lo = voronoi3D[0];
		int Vx_hi = voronoi3D[1];
		int Vy_lo = voronoi3D[2];
		int Vy_hi = voronoi3D[3];
		int Vz_lo = voronoi3D[4];
		int Vz_hi = voronoi3D[5];
		int Vnp = voronoi3D[6];
		int Vsz = voronoi3D[7];
		Print()<<"Vx_lo: "<<Vx_lo<<"\n";
		Print()<<"Vx_hi: "<<Vx_hi<<"\n";
		...
		amrex::Vector<Real> flag(ncellx*ncelly*ncellz,0.0);
		amrex::Vector<Real> phi(ncellx*ncelly*ncellz,0.0);
		amrex::Vector<Real> n(Vnp,0.0);
		amrex::Vector<Real> m(Vnp,0.0);
		amrex::Vector<Real> p(Vnp,0.0);
		amrex::Vector<int> phase(Vnp,0.0);

		long limit_x = Vx_hi-Vx_lo;
		long limit_y = Vy_hi-Vy_lo;
		long limit_z = Vz_hi-Vz_lo;

		Print()<<"init done \n";

		for(int i=0; i<Vnp; i++){
        while(Phase_filled!=1){
            rand_x = Vx_lo+drand48()*(limit_x-1);
            rand_y = Vy_lo+drand48()*(limit_y-1);
            rand_z = Vz_lo+drand48()*(limit_z-1);
            gidy = rand_x*ncelly*ncellz + rand_z*ncelly + rand_y;
            ...
            if(flag[gidy]!=1){
                ...
                m[i] = rand_y;
				p[i] = rand_z;
				//Print()<<"inside if: "<<n[i]<<" "<<m[i]<<" "<<p[i]<<"\n";

            for(int x=0; x<ncellx; x++){
                for(int y=0; y<ncelly; y++){
                    for(int z=0; z<ncellz; z++){
                        if ((n[i]-x)*(n[i]-x) +(m[i]-y)*(m[i]-y) + (p[i]-z)*(p[i]-z) <= Vsz*Vsz) {
                            gidy1 = x*ncelly*ncellz + z*ncelly + y;
                            flag[gidy1] = 1;
							//Print()<<"gidy1: "<<gidy1<<"\n";
                        }
            }
		...
		for (int x = 0; x < ncellx; x++) {
			for (int y = 0; y < ncelly; y++) {
				for (int z = 0; z < ncellz; z++) {
					gidy = x*ncelly*ncellz + y*ncellz + z;
					for (int r = 0; r < Vnp; r++) {
						l[r] = (n[r] - x) * (n[r] - x) + (m[r] - y) * (m[r] - y) + (p[r] - z) * (p[r] - z);
					}
...
{ 	
	...
	for (MFIter mfi(mu_new); mfi.isValid(); ++mfi)
	{
		...
		//Print()<<"fin3\n";
		Array3D<Real,0,phasecount-1,0,phasecount-1,0,compcount-2> cfill{};
		for(int a=0; a<nump; a++){
			for(int b=0; b<nump; b++){
				for(int l=0; l<numcom-1; l++){
					cfill(a,b,l) = c_fill[a][b][l];
					//Print()<<"ceq_liq["<<l<<"]: "<<ceq_liq(l)<<"\n";
				}
			}
	    }

		Array2D<Real,0,phasecount-1,0,compcount-2> ceq_sol{};
		for(int a=0; a<nump; a++){
			for(int l=0; l<numcom-1; l++){
				ceq_sol(a,l) = conceq_s[a][l];
				//Print()<<"ceq_sol["<<a<<","<<l<<"]: "<<ceq_sol(a,l)<<"\n";
		 	}
		}
		...
		{	
			...
			if(val>=0){
				//Print()<<"val in two:"<<val<<"\n";
				for(int l=0; l<numcomp-1; l++){
					for(int m=0; m<numcomp-1; m++){
				
						if(l==m){
							sum += 2.0*A_fill(val,l,m)*ceq_sol(val,l);
						}
						else{
							sum += A_fill(val,l,m)*ceq_sol(val,l);
						}
					//	Print()<<"A["<<val<<","<<l<<","<<m<<"]: "<<A_fill(val,l,m)<<"\n";
					//	Print()<<"ceq_sol["<<val<<","<<m<<"]: "<<ceq_sol(val,m)<<"\n";
					}
					muNew(i,j,k,l) = sum + BB(val,l);
					//Print()<<"BB["<<val<<","<<l<<"]: "<<BB(val,l)<<"\n";
					sum=0.0;
				}
			}
			...
			//Print()<<"muNew[ "<<i<<","<<j<<","<<k<<",0]: "<<muNew(i,j,k,0)<<"\n";
		});
	}
}	
